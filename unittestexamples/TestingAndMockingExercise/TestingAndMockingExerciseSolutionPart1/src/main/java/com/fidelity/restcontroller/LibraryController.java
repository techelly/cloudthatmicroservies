...
// import com.fidelity.integration.LibraryDao;  // not required after the controller uses the business service
...
public class LibraryController {
	...
	@Autowired
	private LibraryBusinessService businessService;
	...
	public ResponseEntity<BookList> getBooks() {
		...
		try {
			List<Book> books = businessService.getAllBooks();
			// List<Book> books = dao.getBooks(); // DAO replaced by business service
	...
	public ResponseEntity<Book> getBookById(@PathVariable String isbn) {
		try {
			...
			Book book = businessService.getBookById(isbn);
			// Book book = dao.getBookByIsbn(isbn); // DAO replaced by business service
	...
	// TODO: Move the following method to the LibraryBusinessService.
